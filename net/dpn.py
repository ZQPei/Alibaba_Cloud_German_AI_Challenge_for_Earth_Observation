...
class DPN(nn.Module):
    ...
    def __init__(self, num_init_features=64, k_R=96, G=32,
                 k_sec=(3, 4, 20, 3), inc_sec=(16,32,24,128), num_classes=17):
        ...
        # )
        blocks['conv1'] = nn.Sequential(
            nn.Conv2d(10, num_init_features, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(num_init_features),
            nn.ReLU(inplace=True),
            # nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(features, kernel_size=4).view(features.size(0), -1)
